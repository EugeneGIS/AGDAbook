...
As part of the GNU Project - free software, mass collaboration project - <https://www.gnu.org/home.en.html>, the source code is free available.
Specific functionalities in R can be expanded by importing packages, which include a set of R functions, compiled code, and sample data designed for specific purposes.
For more details on R see <https://www.r-project.org/>.

### 1.1) R Packages

A package is a file generally composed of R scripts (e.g., functions). It includes R functions, data, and compiled code, all stored in a location known as the *library*.
...
For example, to check the list of the installed packeges, the function `library()` can be used.
If you need a specific functionality, you can download the corresponding package from a suitable repository, and it will be added to your library. To utilize the package, you use the command library(package), which loads the package for use in your session.
NB: when you open an **R Markdown** document (*.Rmd*) the program propose you automatically to install the libraries listed there.
...
### 1.3) R Commands (online resources)
...
## 3) Data type in computational analysis
