...
# DBSCAN for 3D fetures detection in point clouds
...
The present lab deals with steps 2 and 3: 3D features detection using DBSCAN.
...
Once fixed the $MinPts$ parameter, a suitable value for the $eps$ neighborhood size can be estimated using k-nearest neighbors (k-NN) distance graph, imposing k as equal to $MinPts$ and plotting the distance to the nearest neighbors.
