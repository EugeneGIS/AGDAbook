...
subtitle: 'Landslides susceptibility mapping using Random Forest'
editor_options:
...
You are asked to carefully read each explanatory paragraph and than to run each individual R code chunk, one by one, and to interpret the results.
Finally you can start again from the beginning of the exercise and change some parameters as indicated below.
...
library(readr) #The goal of 'readr' is to provide a fast and friendly way to read rectangular data (like 'csv', 'tsv', and 'fwf').
library(randomForest) #Classification and regression based on a forest of trees using random inputs, based on Breiman (2001) <doi:10.1023/A:1010933404324>.
library(dplyr) #dplyr is the next iteration of plyr, focussed on tools for working with data frames (hence the d in the name).
library(pROC) # Allowing to compute, analyze ROC curves
library(plotROC) #and display ROC curve
library(ggplot2) #Is a system for declaratively creating graphics, 
...
landCover<-as.factor(rast("landCover.tif"))
geology<-as.factor(rast("Geological_classes.tif"))

planCurv<-rast("plan_curvature.tif")/100
profCurv<-rast("profil_curvature.tif")/100
# this because ArcGIS multiply curvature values by 100

TWI<-rast("TWI.tif")
Slope<-rast("Slope.tif")
dem<-rast("DEM.tif")
dist<-rast("dist_roads.tif")
...
### 2.2.1) The use of categorical variables in Machine Learning
...
knitr::include_graphics("Cat_classes.png")
```
...
### 2.3.1) Split the input dataset into training (80%) and testing (20%)
...
## 5.1) Class intervals for decision maker
