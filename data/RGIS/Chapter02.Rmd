---
title: "Basic operation with geodata in R"
...
## 2.1) Load libraries and vector dataset
...
```{r load-shp}
# Load dataset
vaud <- st_read("./Lab002/Canton_de_Vaud.shp", quiet = FALSE)

```

## 2.2) Plot vector features
...
## 3.2) Load libraries and raster dataset
...
Vaud_dem <- rast("./Lab002/DEM.tif")

# Inspect the raster
Vaud_dem
```

## 3.2) Plot raster features
...
## 4.1) Manipulate tabular datasets
...
Swisscensus_2020 <- read.csv("./Lab002/census2020.csv")
...
## 4.2) Pipes: chaining of multiple operations
...
## 4.6) Display categorigal variables for a raster 
...
# Load the raster data
landCover <- rast("./Lab002/landCover.tif")
...
# Convert the raster to a factor to represent categorical data
landCover <- as.factor(rast("./Lab002/landCover.tif"))
